...
Класс Cat, реализующий Meowing:
   - Приватные поля:
        - name: String
        - meowCount: int
   - Конструктор:
        - Параметр: name (String)
        - Проверить, что name не null и не пустое
        - Инициализировать поле name
        - Инициализировать meowCount = 0
   - Метод meow():
        - Вывести на экран: "name: мяу!"
        - Увеличить meowCount на 1
   - Метод toString():
        - Вернуть строку: "кот: name"
   - getName(): возвращает name
   - getMeowCount(): возвращает текущее значение meowCount

Класс Task2_1 с методом makeAllMeow():
   - Метод makeAllMeow(List<Meowing> meowingObjects):
     - параметры:
          - meowingObjects: список объектов, реализующих интерфейс Meowing
     - вызов метода meow()
